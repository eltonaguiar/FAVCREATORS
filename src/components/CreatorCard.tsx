    ...
    const getPlatformIcon = (platform: string) => {
        switch (platform) {
            ...
            case 'instagram': return (
                <svg viewBox="0 0 24 24" className="platform-icon" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        <div className="creator-card">
            ...
            <div style={{ position: 'absolute', top: '15px', right: '15px', display: 'flex', gap: '8px', alignItems: 'center' }}>
                <button
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '1rem', opacity: checking ? 0.3 : 0.6 }}
                >
                    {checking ? '⏳' : '📡'}
                </button>
                <button
                ...
                {creator.accounts.map(account => (
                    <div key={account.id} className={`account-link ${account.platform}`}>
                        <a
                            ...
                            style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', textDecoration: 'none', color: 'inherit' }}
                        >
                            {getPlatformIcon(account.platform)}
                            <div className="account-info">
                                <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
                                    <span>{account.platform === 'other' ? account.username : `@${account.username}`}</span>
                                    {account.isLive && <div className="account-live-dot"></div>}
                                </div>
                                {account.followers && <span className="follower-count">{account.followers} followers</span>}
                            </div>
                        </a>
                        <button
